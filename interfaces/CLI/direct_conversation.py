...
class CLI:
    ...
    async def _process_user_input(self, user_input: str):
        ...
        if result.slash_commands_processed:
            self.console.print(
                f"✅ Processed commands: {result.slash_commands_processed}", style="green"
            )
        ...
        if result.resume_command:
            ...
            self.console.print(
                f"🔄 Starting new conversation turn (execution_step_results: {len(step_results)} records)...",
                style="blue",
            )
    ...
    async def _show_final_result(self, result: Dict[str, Any]):
        ...
        self.console.print(
            f"📊 Execution completed (execution_step_results: {len(step_results)} records)",
            style="cyan",
        )
...
async def async_main():
    ...
    Creates a CLI instance and starts the interactive session. This async function
    serves as the core entry point that handles the CLI lifecycle.
...
def main():
    ...
    This function serves as the entry point for the alpha-berkeley CLI command
    when installed via pip. It wraps the async_main() function to work with
